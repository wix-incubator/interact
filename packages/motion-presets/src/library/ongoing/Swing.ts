...
export function style(options: TimeAnimationOptions & AnimationExtraOptions, asWeb = false) {
  const { power, swing = 20, direction = DEFAULT_DIRECTION } = options.namedEffect as Swing;
  ...
  const swingDeg = power && power in POWER_TO_SWING_FACTOR_MAP ? getMapValue(POWER_TO_SWING_FACTOR_MAP, power, POWER_TO_SWING_FACTOR_MAP[power]) * 20 : swing;
