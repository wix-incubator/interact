...
.target {
  --_interact-delay: calc(pow(var(--interact-seq-i) / var(--interact-seq-c), 2)); /* quadIn - this is here for readability, don't actually have to add as a separate property */
  animation-delay: calc(effectDelay + var(--_interact-delay) * var(--interact-seq-c) * <sequence.offset>);
}
```
